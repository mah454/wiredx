...
abstract class IO{
  ...
  abstract void writeShort(int val) throws IOException;
  abstract  void writeInt(int val) throws IOException;
  ...
  void readByte(byte[] array) throws IOException{
    readByte(array, 0, array.length);
  }
  void readByte(byte[] array, int begin, int length) throws IOException{
    int i=0;
    while(true){
      if((i=(inrest))<length){
	if(i!=0){
  ...
  void writeByte(byte[] array) throws IOException{
    writeByte(array, 0, array.length);
  }

  void writeByte(byte[] array, int begin, int length) throws IOException{
    if(length<=0) return;
    int i=0;
    while(true){
  ...
  synchronized void immediateWrite(byte[] array, int b, int l)throws IOException{
    out.write(array, b, l);
  }
