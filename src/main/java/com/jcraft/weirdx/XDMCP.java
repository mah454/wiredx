...
//	socket = new DatagramSocket();
    ...
    void putByte(int val) throws IOException{
      putByte((byte)val);
    }

    void putByte(byte[] array) throws IOException{
      putByte(array, 0, array.length);
    }

    void putByte(byte[] array, int begin, int length) throws IOException{
      if(length<=0) return;
      int i=0;
      while(true){
    ...
    void putShort(int val) throws IOException{
